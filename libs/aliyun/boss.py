...
class EcsAPi():
    ...
    def get_server_info(self):
        ...
        for i in ret:
            ...
            asset_data['expired_time'] = i.get('ExpiredTime')
            # 内网IP
            try:
                #VPC里面内网IP
            ...
            for server in server_list:
                ...
                if exist_ip:
                    session.query(ServerDetail).filter(ServerDetail.ip == ip).update(
                        {ServerDetail.instance_id: instance_id, ServerDetail.instance_type: instance_type,ServerDetail.expired_time:expired_time,
                    ...
                    if os_type == 'windows':
                        ...
                        new_serve_detail = ServerDetail(ip=ip, instance_id=instance_id, instance_type=instance_type,
                                                        ...
                                                        memory=memory, os_type=os_name, disk=disk, os_kernel=os_kernel,expired_time=expired_time,
                                                        sn=sn)
...
def main():
    ...
    for config in aliyun_configs_list:
        ...
        response_data = obj.index()
        s = {}
        if response_data:
            s["provider_id"] = provider_id
            s["Currency"] = response_data["Data"]["Currency"]
            s["AvailableCashAmount"] = response_data["Data"]["AvailableCashAmount"]
            ret.append(s) 
    return ret
